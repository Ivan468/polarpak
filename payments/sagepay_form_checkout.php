...
/*
  ...
  ***                                                                      ***
  ***      Viart Shop 5.6                                                  ***
  ***      File:  sagepay_form_checkout.php                                ***
 ...
 */
	...
	// disable state parameter for non US countries
	$billing_country = get_setting_value($payment_parameters, "billingcountry", "");
	$billing_country = get_setting_value($payment_parameters, "BillingCountry", $billing_country);
	if (strtoupper($billing_country) != "US") {
		if(isset($payment_parameters["BillingState"])) { $payment_parameters["BillingState"] = ""; } 
		if(isset($payment_parameters["billingstate"])) { $payment_parameters["billingstate"] = ""; } 
	}
	$delivery_country = get_setting_value($payment_parameters, "deliverycountry", "");
	$delivery_country = get_setting_value($payment_parameters, "DeliveryCountry", $delivery_country);
	if (strtoupper($delivery_country) != "US") {
		if(isset($payment_parameters["DeliveryState"])) { $payment_parameters["DeliveryState"] = ""; } 
		if(isset($payment_parameters["deliverystate"])) { $payment_parameters["deliverystate"] = ""; } 
	}
	...
	$pass_data['Crypt'] = get_sagepay_crypt($payment_parameters);
