 ...
 */
	...
	if ($ch) 
	{
		...
		if ($payment_response) {
			...
			$variables["authorization_code"] = $response_parameters[4];
			if ($response_parameters[0] == 4) {
				$pending_message  = "Your transaction is being held for review.";
				$pending_message .= strlen($response_parameters[2]) ? " Response Reason Code: " . $response_parameters[2] : ""; 
			} else if ($response_parameters[0] != 1) {
