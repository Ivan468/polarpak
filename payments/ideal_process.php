 ...
 */
	...
	else 
	{
		...
		if (!isset($variables["order_total_100"])) { $variables["order_total_100"] = $variables["order_total"] * 100; } 		
		...
		if ($payment_parameters["IssuerID"] == "") {
			...
			$issuers = isset($response_parameters["IssuerID"]) ? $response_parameters["IssuerID"] : array();
			if (sizeof($issuers)==0) {
				echo "Can't find any issuers.";
				echo htmlspecialchars($answer);
				exit;
			} else if (sizeof($issuers)==1) {
				$payment_parameters["IssuerID"] = $response_parameters["IssuerID"][0][0];
			} else {
?>
<html>
<head>
