 ...
 */
	...
	if ($payment_response) {
		...
		if (!$keyfile) {
			$error_message = "'keyfile' parameter is required for LinkPoint API.";
		} else if (!file_exists($keyfile) || preg_match("/^https?:/", $keyfile)) {
			$error_message = "Can't find LinkPoint SSL certificate, please use absolute path like '/home/user_name/keys/1234567.pem' for keyfile payment parameter.";
		} else if (!@fopen($keyfile, "r")) {
			$error_message = "Can't read LinkPoint SSL certificate, please check read permissions to the keyfile.";
		} else {
			$error_message = "Empty response from LinkPoint, please check that your payment parameters: configfile and keyfile were set correctly and outgoing connections are not blocked by firewall.";
		}
	}
