 ...
 */
	...
	if (strlen($paypal_response)) {
		...
		$phone = "";
		if (preg_match("/<Phone[^>]*>(.*)<\/Phone>/i", $paypal_response, $match)) {
			$phone = trim($match[1]);
		}
		if (!$phone && preg_match("/<ContactPhone[^>]*>(.*)<\/ContactPhone>/i", $paypal_response, $match)) {
			$phone = trim($match[1]);
		}
		...
		$sql  = " SELECT is_fast_checkout FROM " . $table_prefix . "orders ";
		$sql .= " WHERE order_id=" . $db->tosql($order_id, INTEGER) ;
		$is_fast_checkout = get_db_value($sql);

		// update order data only if user use fast checkout option
		if ($is_fast_checkout) {
			...
			if (strlen($phone)){
				$sql .= ", phone=" . $db->tosql($phone, TEXT) ;
				$sql .= ", daytime_phone=" . $db->tosql($phone, TEXT) ;
			}
			...
			// update province if it wasn't set
			if(strtoupper($country_code) != "US") {
				$sql  = " UPDATE " . $table_prefix . "orders ";
				$sql .= " SET province=" . $db->tosql($province, TEXT) ;
				$sql .= " WHERE order_id=" . $db->tosql($order_id, INTEGER) ;
				$sql .= " AND (province IS NULL OR province='') ";
				$sql .= " AND (state_id IS NULL OR state_id=0) ";
				$db->query($sql);

				$sql  = " UPDATE " . $table_prefix . "orders ";
				$sql .= " SET delivery_province=" . $db->tosql($delivery_province, TEXT) ;
				$sql .= " WHERE order_id=" . $db->tosql($order_id, INTEGER) ;
				$sql .= " AND (delivery_province IS NULL OR delivery_province='') ";
				$sql .= " AND (delivery_state_id IS NULL OR delivery_state_id=0) ";
				$db->query($sql);
			}
		}
