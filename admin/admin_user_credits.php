...
*/
	...
	$t->set_var("CONFIRM_DELETE_JS", str_replace("{record_name}", CREDITS_MSG, CONFIRM_DELETE_MSG));
	...
	for($i = 0; $i < sizeof($types); $i++) {
		...
		$credits_today = doubleval(get_db_value($sql));
		...
		$credits_yesterday = doubleval(get_db_value($sql));
		...
		$credits_week = doubleval(get_db_value($sql));
		...
		$credits_month = doubleval(get_db_value($sql));
