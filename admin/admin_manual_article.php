...
*/
	...
	if ($article_id && !$manual_id) {
		$sql  = " SELECT manual_id FROM ".$table_prefix."manuals_articles ";
	...
	$html_editor = get_setting_value($settings, "html_editor_manuals", get_setting_value($settings, "html_editor", 1));
	...
	$r->change_property("parent_article_id", DEFAULT_VALUE, $parent_article_id);
	...
	if (empty($options)) {
		...
		// Check if any error occurred and have to set options of selected parent not default
