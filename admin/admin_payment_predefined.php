...
*/
	...
	if (file_exists($db_file)) {
		...
		while (!feof($fp)) {
			$sql_string = fgets($fp);
			if (preg_match("/^insert\s+into\s+\[?va_payment_systems\]?\s+\([^)]+\)\s+values\s+\(\s*(\d+)\s*,\s*(\d+)\s*,\s*'([^']+)'/i", $sql_string, $matches)) {
				$row_payment_id = $matches[1];
				if ($operation == "apply" && $row_payment_id == $payment_id) {
					$payment_sql = $sql_string;
				}
				$payment_systems[] = array($row_payment_id, $matches[3]);
			}
		}
		fclose($fp);
	} else {
	...
	if (!$errors && $operation == "apply") {
		if ($payment_id) {
			...
			// remove semicolon and space symbols in the end
			$payment_sql = str_replace("va_payment_systems", $table_prefix."payment_systems", $payment_sql);
			// add predefined payment system as new system and new id
			if ($db->query($payment_sql)) {
				$new_payment_id = $db->last_insert_id();
				...
				while (!feof($fp)) {
					...
					if (preg_match("/^INSERT\s+INTO\s+va_payment_parameters\s+\([^)]+\)\s+VALUES\s+\(\s*(\d+)\s*,\s*".$payment_id."\s*,/i", $sql_string, $matches)) {
						...
						$sql = str_replace("va_payment_parameters", $table_prefix."payment_parameters", $sql);
						$db->query($sql);
						...
						$sql = str_replace("va_global_settings", $table_prefix."global_settings", $sql);
						$db->query($sql);
					}
				}
				fclose($fp);
