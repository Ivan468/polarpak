...
*/
	...
	{
		$sql_where  = " WHERE block_name LIKE '%".$db->tosql($search_block, TEXT, false)."%' ";
		$sql_where .= " OR block_title LIKE '%".$db->tosql($search_block, TEXT, false)."%' ";
		$sql_where .= " OR block_desc LIKE '%$".$db->tosql($search_block, TEXT, false)."%' ";
	...
	$s->set_sorter(ID_MSG, "sorter_block_id", "1", "block_id", "", "", true);
	...
	if($db->next_record())
	{
		...
		do
		{
			...
			if (strlen($block_notes) > 100) {
				$block_notes = substr($block_notes, 0, 96)." ...";
			}
