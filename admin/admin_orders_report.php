...
*/
	...
	if ($db_type == "access" || $db_type == "postgre" || $db_type == "sqlsrv") 
	{
		...
		$s->set_sorter(PROD_SHIPPING_MSG, "sorter_shipping", "5", "SUM(o.shipping_cost), SUM(o.shipping_excl_tax)");
	...
	elseif ($db_type == "mysql") 
	{
		...
		$s->set_sorter(PROD_SHIPPING_MSG, "sorter_shipping", "5", "shipping", "shipping ASC, shipping_excl_tax ASC", "shipping DESC, shipping_excl_tax DESC");
	...
	$sql = "";
	if ($db_type == "access" || $db_type == "sqlsrv" || $db_type == "db2") {
		$sql = "SELECT Year(o.order_placed_date) AS year, ";
		if ($s_gr == 2) {
			$sql .= "Month(o.order_placed_date) AS month, ";
		} else if ($s_gr == 3) {
			if ($db_type == "sqlsrv") {
				$sql .= "DatePart(ww, o.order_placed_date) AS week, ";
			} else {
				$sql .= "DatePart('ww', o.order_placed_date) AS week, ";
			}
		} else if ($s_gr == 4) {
	...
	elseif ($db_type == "mysql") 
	{
		...
		switch ($s_gr) {
			case 2:
				...
				break;
			case 3:
				$sql .= " WEEK(o.order_placed_date) AS week, ";
				break;
			case 4:
		...
		switch ($s_gr) {
			case 2:
				...
				break;
			case 3:
				$sql .= " date_part('week', o.order_placed_date)  AS week, ";
				break;
			case 4:
	...
	if ($db_type == "access" || $db_type == "sqlsrv" || $db_type == "db2") {
		if ($s_gr == 2) {
			...
			if ($db_type == "sqlsrv") {
				$sql .= "Year(o.order_placed_date), DatePart(ww, o.order_placed_date) ";
			} else {
				$sql .= "Year(o.order_placed_date), DatePart('ww', o.order_placed_date) ";
			}
		} else if ($s_gr == 4) {
	...
	if ($db->next_record()) {
		...
		do {
			...
			if ($s_gr == 1) {
				...
				if ($db_type == "postgre") { 
					// NOTE: in Postgre week always starts from Monday
					$year_start_weekday = date("N", mktime(0, 0, 0, 1, 1, $year)); // ISO-8601 numeric representation of the day of the week 1 - 7 (added in PHP 5.1.0)
					$year_start_weekday--;
					if ($year_start_weekday >= 5) {
						$day_number = 7 * ($week + 1) - $year_start_weekday;
					} else {
						$day_number = 7 * $week - $year_start_weekday;
					}
				} else {
					$year_start_weekday = date("w", mktime(0, 0, 0, 1, 1, $year)); // Numeric representation of the day of the week 0 - 6
