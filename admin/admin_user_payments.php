...
*/
	...
	for($i = 0; $i < sizeof($types); $i++) {
		...
		$payments_today = doubleval(get_db_value($sql));
		...
		$payments_yesterday = doubleval(get_db_value($sql));
		...
		$payments_week = doubleval(get_db_value($sql));
		...
		$payments_month = doubleval(get_db_value($sql));
	...
	if($db->next_record())
	{
		...
		do {
			...
			$is_paid = ($db->f("is_paid") == 1) ? "<b>".YES_MSG."</b>" : NO_MSG;
