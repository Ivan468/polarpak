...
*/
	...
	if($operation == "upload")
	{
		...
		{
			// get attachments dir 
			$attachments_dir = ""; $sub_attachments_dir = "";
			if ($dep_id) {
			...
			if (!$attachments_dir) {
				$attachments_dir = "support_attachments";
			}
			if (!preg_match("/^[\/\\\\]/", $attachments_dir) && !preg_match("/\:/", $attachments_dir)) {
				$sub_attachments_dir = "../";
			}
			...
			$file_index = 0;
			while (file_exists($sub_attachments_dir.$filepath . $new_filename)) {
				$file_index++;
			...
			if(!@move_uploaded_file($tmp_name, $sub_attachments_dir.$filepath . $new_filename)) {
				if (!is_dir($sub_attachments_dir.$filepath)) {
					$errors = FOLDER_DOESNT_EXIST_MSG . $sub_attachments_dir.$filepath ;
				} else if (!is_writable($sub_attachments_dir.$filepath)) {
					$errors = str_replace("{folder_name}", $sub_attachments_dir.$filepath, FOLDER_PERMISSION_MESSAGE);
				} else {
					$errors = UPLOAD_CREATE_ERROR . $sub_attachments_dir.$filepath . $filename . "</b>";
				}
			} else {
				chmod($sub_attachments_dir.$filepath . $new_filename, 0766);
