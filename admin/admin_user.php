...
*/
	...
	// check all available sites for user 
	$user_site_ids = array(); $user_sites_all = false;
	$sql  = " SELECT site_id FROM " . $table_prefix . "user_types_sites ";
	$sql .= " WHERE type_id=" . $db->tosql($type_id, INTEGER);
	$sql .= " ORDER BY site_id ASC ";
	$db->query($sql); 
	if ($db->next_record()) {
		$user_site_id = $db->f("site_id");
		$user_site_ids[] = $user_site_id;
	}
	...
	while ($db->next_record()) {
		$user_type_settings[$db->f("setting_name")] = $db->f("setting_value");
	}
	$affiliate_join = get_setting_value($user_type_settings, "affiliate_join", 0);
	...
	if (!$user_sites_all) {
		$sql .= " AND (site_id=1 ";
		if (count($user_site_ids)) {
			$sql .= " OR site_id IN (" . $db->tosql($user_site_ids, INTEGERS_LIST). ") ";
		}
		$sql .= " ) ";
	}
	$sql .= " ORDER BY site_id ASC ";
	...
	while ($db->next_record()) {
		...
		if (!isset($user_profile[$setting_name])) {
			$user_profile[$setting_name] = $setting_value;
		} else if (isset($user_profile[$setting_name]) && !$user_profile[$setting_name]) {
			$user_profile[$setting_name] = $setting_value;
		}
	}
	...
	if ($db->next_record()) {
		do {
			...
			$pp[$pn]["property_class"] = $db->f("property_class");
	...
	$r->change_property("affiliate_code", BEFORE_SHOW, "affiliate_code_help");
	if ($affiliate_join) {
	...
	$r->add_textbox("user_settings", TEXT);
	// special settings record to override different global settings
	$sr = new VA_Record("", "sr");
	...
	$sr->add_radio("voucher_purchase", INTEGER, $yes_no_default);
	$sr->add_textbox("purchase_fee_percent", NUMBER, va_constant("PURCHASE_FEE_MSG")." - ".va_constant("PERCENTAGE_MSG"));
	$sr->add_textbox("purchase_fee_amount", NUMBER, va_constant("PURCHASE_FEE_MSG")." - ".va_constant("FIXED_AMOUNT_MSG"));

	$sr->add_radio("voucher_transfer", INTEGER, $yes_no_default);
	$sr->add_textbox("transfer_time_limit", INTEGER);
	$sr->add_textbox("transfer_fee_percent", NUMBER, va_constant("TRANSFER_FEE_MSG")." - ".va_constant("PERCENTAGE_MSG"));
	$sr->add_textbox("transfer_fee_amount", NUMBER, va_constant("TRANSFER_FEE_MSG")." - ".va_constant("FIXED_AMOUNT_MSG"));

	$sr->add_radio("cash_out_voucher", INTEGER, $yes_no_default);
	$sr->add_textbox("cash_out_time_limit", INTEGER);
	$sr->add_textbox("cash_out_fee_percent", NUMBER, va_constant("CASH_OUT_FEE_MSG")." - ".va_constant("PERCENTAGE_MSG"));
	$sr->add_textbox("cash_out_fee_amount", NUMBER, va_constant("CASH_OUT_FEE_MSG")." - ".va_constant("FIXED_AMOUNT_MSG"));
	$sr->add_select("cash_out_status_id", INTEGER, $order_statuses, va_constant("ORDER_STATUS_MSG"));
	...
	$r->events[BEFORE_INSERT] = "prepare_data";
	$r->events[BEFORE_UPDATE] = "prepare_data";
	...
	$r->events[BEFORE_DELETE] = "delete_user_data";
	$r->events[AFTER_VALIDATE] = "validate_settings";
	...
	// get user settings parameters
	$sr->get_form_parameters();
	$r->process();

	$t->set_var("type_id", htmlspecialchars($type_id));

	// get default voucher settings
	$yes_no = array("0" => va_constant("NO_MSG"), "1" => va_constant("YES_MSG"));
	$voucher_purchase = get_setting_value($voucher_settings, "voucher_purchase", 0);
	$purchase_fee_percent = get_setting_value($voucher_settings, "purchase_fee_percent", 0);
	$purchase_fee_amount = get_setting_value($voucher_settings, "purchase_fee_amount", 0);

	$voucher_transfer = get_setting_value($voucher_settings, "voucher_transfer", 0);
	$transfer_time_limit = get_setting_value($voucher_settings, "transfer_time_limit", 0);
	$transfer_fee_percent = get_setting_value($voucher_settings, "transfer_fee_percent", 0);
	$transfer_fee_amount = get_setting_value($voucher_settings, "transfer_fee_amount", 0);

	$cash_out_voucher = get_setting_value($voucher_settings, "cash_out_voucher", 0);
	$cash_out_time_limit = get_setting_value($voucher_settings, "cash_out_time_limit", 0);
	$cash_out_fee_percent = get_setting_value($voucher_settings, "cash_out_fee_percent", 0);
	$cash_out_fee_amount = get_setting_value($voucher_settings, "cash_out_fee_amount", 0);
	$cash_out_status_id = get_setting_value($voucher_settings, "cash_out_status_id", "");

	// set default voucher settings
	$t->set_var("default_voucher_purchase", $yes_no[$voucher_purchase]);
	$t->set_var("default_purchase_fee_percent", $purchase_fee_percent."%");
	$t->set_var("default_purchase_fee_amount", currency_format($purchase_fee_amount));

	$t->set_var("default_voucher_transfer", $yes_no[$voucher_transfer]);
	$t->set_var("default_transfer_time_limit", $transfer_time_limit);
	$t->set_var("default_transfer_fee_percent", $transfer_fee_percent."%");
	$t->set_var("default_transfer_fee_amount", currency_format($transfer_fee_amount));

	$t->set_var("default_cash_out_voucher", $yes_no[$cash_out_voucher]);
	$t->set_var("default_cash_out_time_limit", $cash_out_time_limit);
	$t->set_var("default_cash_out_fee_percent", $cash_out_fee_percent."%");
	$t->set_var("default_cash_out_fee_amount", currency_format($cash_out_fee_amount));
	$t->set_var("default_cash_out_status", get_array_value($cash_out_status_id, $order_statuses));

	// set user settings
	$sr->set_form_parameters();
	...
	$tabs = array(
		...
		"voucher_settings" => array("title" => USER_VOUCHER_SETTINGS_MSG), 
		"stats" => array("title" => ADMIN_STATISTIC_MSG, "show" => $stats), 
		"order_restrictions" => array("title" => ORDER_RESTRICTIONS_MSG), 
		"orders_items" => array("title" => ORDER_ITEMS_MSG, "show" => $total_order_items), 
	...
	function prepare_data($params)  {
		global $db, $table_prefix, $r, $sr, $settings, $user_profile, $type_id;
		...
		if ($event == BEFORE_INSERT) {
			...
			$show_email = get_setting_value($user_profile, "show_email", 0);
			$login_field_type = get_setting_value($user_profile, "login_field_type", 1);
			// get email from login data
			if ($login_field_type == 2 && !$show_email) {
		...
		// save user settings in json format
		$user_settings = array();
		foreach ($sr->parameters as $key => $data) {
			if (strlen($data[CONTROL_VALUE])) {
				$user_settings[$key] = $data[CONTROL_VALUE];
			}
		}
		$r->set_value("user_settings", json_encode($user_settings));
	}
	...
	{
		global $r, $sr, $pp, $db, $table_prefix;
		...
		// decode user setting and set to control
		$user_settings = $r->get_value("user_settings");
		if ($user_settings) {
			$user_settings = json_decode($user_settings, true);
			foreach ($user_settings as $param_name => $param_value) {
				if ($sr->parameter_exists($param_name)) {
					$sr->set_value($param_name, $param_value);
				}
			}	
		}
	}
	...
	function delete_user_data()
	{
		...
		$user_id = $r->get_value("user_id");
		// delete user properties and properties
		$sql  = " DELETE FROM " . $table_prefix . "users_properties WHERE user_id=" . $db->tosql($user_id, INTEGER);
		$db->query($sql);
		$sql  = " DELETE FROM " . $table_prefix . "users_addresses WHERE user_id=" . $db->tosql($user_id, INTEGER);
		$db->query($sql);
		// delete user files and photos
		$sql  = " DELETE FROM " . $table_prefix . "users_files WHERE user_id=" . $db->tosql($user_id, INTEGER);
		$db->query($sql);
		$sql  = " DELETE FROM " . $table_prefix . "users_photos WHERE user_id=" . $db->tosql($user_id, INTEGER);
		$db->query($sql);
		// delete user commissions only if they weren't paid
		$sql  = " DELETE FROM " . $table_prefix . "users_commissions ";
		$sql .= " WHERE user_id=" . $db->tosql($user_id, INTEGER);
		$sql .= " AND payment_id=0 ";
		$db->query($sql);
		// do not delete payments to keep all payments history
		//$sql  = " DELETE FROM " . $table_prefix . "users_payments WHERE user_id=" . $db->tosql($user_id, INTEGER);
		//$db->query($sql);
		// delete user credits, points 
		$sql  = " DELETE FROM " . $table_prefix . "users_credits WHERE user_id=" . $db->tosql($user_id, INTEGER);
		$db->query($sql);
		$sql  = " DELETE FROM " . $table_prefix . "users_points WHERE user_id=" . $db->tosql($user_id, INTEGER);
		$db->query($sql);
		$sql  = " DELETE FROM " . $table_prefix . "users_ps_details WHERE user_id=" . $db->tosql($user_id, INTEGER);
		$db->query($sql);
		$sql  = " DELETE FROM " . $table_prefix . "users_ps_properties WHERE user_id=" . $db->tosql($user_id, INTEGER);
		$db->query($sql);
	}
	...
	function affiliate_code_help()
	{
		...
		$affiliate_code_help = str_replace("{affiliate_url}", $affiliate_url, va_message("AFFILIATE_CODE_HELP_MSG"));
