...
*/
	...
	$styles[] = array();
	$sql = "SELECT setting_value FROM ".$table_prefix."global_settings WHERE setting_name='layout_id'";
	$db->query($sql);
	$db->next_record();
	$layout_id = $db->f("setting_value");
		
	$sql = "SELECT templates_dir, style_name FROM ".$table_prefix."layouts WHERE layout_id=".$db->tosql($layout_id, INTEGER);
	$db->query($sql);
	$db->next_record();
