...
*/
	...
	if ($db->next_record()) {
		do {
			...
			$pp[$pn]["property_type"] = $db->f("property_type");
			...
			$pp[$pn]["before_name_html"] = get_translation($db->f("before_name_html"));
			$pp[$pn]["after_name_html"] = get_translation($db->f("after_name_html"));
			$pp[$pn]["before_control_html"] = get_translation($db->f("before_control_html"));
			$pp[$pn]["after_control_html"] = get_translation($db->f("after_control_html"));
	...
	if ($p_d == 2) {
		...
		$r->add_textbox("delivery_name", TEXT,  NAME_MSG);
		$r->add_textbox("delivery_first_name", TEXT,  FIRST_NAME_FIELD);
		$r->add_textbox("delivery_middle_name", TEXT,  MIDDLE_NAME_FIELD);
		$r->add_textbox("delivery_last_name", TEXT,  LAST_NAME_FIELD);
		$r->add_select("delivery_company_id", INTEGER, $companies,  COMPANY_SELECT_FIELD);
		$r->add_textbox("delivery_company_name", TEXT,  COMPANY_NAME_FIELD);
		$r->add_textbox("delivery_email", TEXT,  EMAIL_FIELD);
		$r->change_property("delivery_email", REGEXP_MASK, EMAIL_REGEXP);
		$r->add_textbox("delivery_address1", TEXT,  STREET_FIRST_FIELD);
		$r->add_textbox("delivery_address2", TEXT,  STREET_SECOND_FIELD);
		$r->add_textbox("delivery_address3", TEXT,  STREET_THIRD_FIELD);
		$r->add_textbox("delivery_city", TEXT,  CITY_FIELD);
		$r->add_textbox("delivery_province", TEXT,  PROVINCE_FIELD);
		$r->add_select("delivery_state_id", INTEGER, $states,  STATE_FIELD);
		$r->change_property("delivery_state_id", USE_SQL_NULL, false);
		$r->add_textbox("delivery_state_code", TEXT);
		$r->add_textbox("delivery_zip", TEXT,  ZIP_FIELD);
		$r->add_select("delivery_country_id", INTEGER, $countries,  COUNTRY_FIELD);
		$r->add_textbox("delivery_country_code", TEXT);
		$r->add_textbox("delivery_phone", TEXT,  PHONE_FIELD);
		$r->add_textbox("delivery_daytime_phone", TEXT,  DAYTIME_PHONE_FIELD);
		$r->add_textbox("delivery_evening_phone", TEXT,  EVENING_PHONE_FIELD);
		$r->add_textbox("delivery_cell_phone", TEXT,  CELL_PHONE_FIELD);
		$r->add_textbox("delivery_fax", TEXT,  FAX_FIELD);
	} elseif ($p_d == 4) {
	...
	$properties_ids = "";
		...
		if ($p_d == 1) {
			...
			$t->set_var("data_title", va_constant("LOGIN_INFO_MSG"));
		} elseif ($p_d == 2) {
			for ($i = 0; $i < sizeof($parameters); $i++)
			{                                    
				$param_name = $parameters[$i];
			...
			$t->set_var("data_title", va_constant("PERSONAL_DETAILS_MSG"));
		} elseif ($p_d == 3) {
			for ($i = 0; $i < sizeof($parameters); $i++)
			{                                    
			...
			$t->set_var("data_title", va_constant("DELIVERY_DETAILS_MSG"));
		} elseif ($p_d == 4) {
			...
			$t->set_var("data_title", va_constant("PAYMENT_DETAILS_MSG"));
		}
		...
		if (sizeof($pp) > 0) 
		{
			for ($pn = 0; $pn < sizeof($pp); $pn++) {
				if ($pp[$pn]["property_type"] == $p_d && ($pp[$pn]["payment_id"] == 0 || $pp[$pn]["payment_id"] == $payment_id)) {
					$section_properties++;
	...
	function encrypt_update_cc_number()
	{
		...
		if ($cc_number_security == 0) {
			...
			$r->set_value("cc_number", va_encrypt(clean_cc_number($r->get_value("cc_number"))));
		}
