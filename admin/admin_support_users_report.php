	...
	// -- generating statuses list
		...
		if($db->next_record()) {
			do {
				...
				$status_ar[$db->f("status_id")] = $status_name;
				$t->parse("statuses",true);
			} while($db->next_record());
			$t->set_var("status_name", "<font>".TOTAL_MSG."</font>"); 
			$t->parse("statuses",true);
			$t->set_var("status_name", "<font>".UNIQUE_MSG."</font>"); 
			$t->parse("statuses",true);
		}
		else {
			$t->set_var("statuses","");
	...
	if (isset($user_ar) && sizeof($user_ar)>0) {
		...
		foreach ($user_ar as $user_name=>$stats) {
		  $row_number +=1;
			...
			$messages .= "<td align='center'><b>".$stats["total"]."</b></td>";
			$messages .= "<td align='center'><b>".$stats["unique"]."</b></td>";
