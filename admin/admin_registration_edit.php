...
*/
	...
	$r->set_event(AFTER_INSERT, "after_save", array("pp" => $pp));
	$r->set_event(AFTER_UPDATE, "after_save", array("pp" => $pp));
	...
	function after_save($params) {
		...
		$event = isset($params["event"]) ? $params["event"] : "";
		$pp = isset($params["pp"]) ? $params["pp"] : array();

		if ($event == AFTER_INSERT) {
			$new_registration = true;
			$registration_id = $db->last_insert_id();
		} else {
		...
		if (($prev_is_approved != $r->get_value("is_approved")) || $event == AFTER_INSERT) {
			send_product_registration_emails($registration_id, $r->get_value("is_approved"), $new_registration);
		}
	}
