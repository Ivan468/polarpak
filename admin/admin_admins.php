...
*/
  ...
  $t->set_file("main","admin_admins.html");
	...
	$sql .= " LEFT JOIN ". $table_prefix . "admin_privileges ap ON a.privilege_id=ap.privilege_id) ";
	if (!$admins_hidden_permission) {
		$sql .= " WHERE (a.is_hidden=0 OR a.is_hidden IS NULL) AND (ap.is_hidden=0 OR ap.is_hidden IS NULL) ";
	}
	$db->query($sql);
	$db->next_record();
