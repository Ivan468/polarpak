...
*/
	...
	$sql  = " SELECT sd.dep_id, sd.short_name ";
	...
	while ($db->next_record()) {
		...
		if ($db->f("is_internal") == "1") {
			$support_statuses[] = array($db->f("status_id"), $status_name . " (Internal)");
		}
		else {
			$support_statuses[] = array($db->f("status_id"), $status_name);
		}
	};
				...
				if ( 1 == 1) {
					...
					if (strtoupper($control_type) == "LISTBOX") {
						...
						if (strlen($operation)) {
							...
							//$control_value = $default_value;
							$control_value = get_db_value(" SELECT property_value FROM ". $table_prefix . "support_properties WHERE property_id=" .$db->tosql($property_id,INTEGER) . " AND support_id=" .$db->tosql($support_id,INTEGER));
						}
						$property_control .= $before_control_html;
