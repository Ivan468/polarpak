  ...
  $t->set_file("main","admin_users_select.html");
	...
	if ($db->next_record()) {
		...
		do {
			...
			$user_name = htmlspecialchars(get_user_name($user_data));
			$user_name_js = str_replace("'", "\\'", $user_name);
			$user_email = htmlspecialchars($db->f("email"));
			$user_email_js = str_replace("'", "\\'", $user_email);
			...
			$t->set_var("user_email_js", $user_email_js);
			$t->set_var("from", $from);
