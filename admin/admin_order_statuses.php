...
*/
	...
	$s->set_sorter(va_message("ID_MSG"), "sorter_status_id", "1", "status_id");
	$s->set_sorter(va_message("ADMIN_ORDER_MSG"), "sorter_status_order", "2", "status_order");
	$s->set_sorter(va_message("STATUS_MSG"), "sorter_status_name", "3", "status_name");
	$s->set_sorter(va_message("ACTIVE_MSG"), "sorter_is_active", "4", "is_active");
	...
	$db->query("SELECT status_id, status_order, status_name, is_active FROM " . $table_prefix . "order_statuses" . $s->order_by);
	if ($db->next_record())
	{
		...
		do {
			$is_active = ($db->f("is_active") == 1) ? "<b>" . va_message("YES_MSG") . "</b>" : va_message("NO_MSG");
			...
			$t->set_var("status_order", $db->f("status_order"));
			$t->set_var("status_name", get_translation($db->f("status_name")));
