...
*/
  ...
  $t->set_file("main","admin_filter_property.html");
	...
	$r->add_textbox("list_field_image", TEXT);
	...
	$ipv->add_textbox("list_image", TEXT, IMAGE_MSG);
...
function update_filter_fields()
{
	...
	if ($property_type == "product_option" && $property_value) {
		...
		$filter_where_sql .= " OR (fipv_".$pi.".property_value='{value_id}' AND (fipv_".$pi.".hide_out_of_stock=0 OR fipv_".$pi.".hide_out_of_stock IS NULL OR fipv_".$pi.".stock_level>0))) ";
		...
		$list_group_where .= " AND (fipv_".$pi.".hide_out_of_stock=0 OR fipv_".$pi.".hide_out_of_stock IS NULL OR  fipv_".$pi.".stock_level>0) ";
