...
*/
	...
	$sql .= " oi.item_id, oi.item_type_id, oi.price, oi.quantity, oi.tax_id, oi.tax_free ";
	...
	if ($dbi->next_record()) {
		...
		do {
			...
			$shipping_cost = doubleval($dbi->f("shipping_cost"));
			...
			$item_tax_id = $dbi->f("tax_id");
			$item_tax_free = $dbi->f("tax_free");
			$item_total = $price * $quantity;

			$item_tax_values = get_tax_amount($order_tax_rates, $item_type_id, $item_total, $quantity, $item_tax_id, $item_tax_free, $item_tax_percent, "", 2, $tax_prices_type, $tax_round);
	...
	function sum_orders_taxes(&$orders_taxes, $order_tax_rates)
	{
		foreach ($order_tax_rates as $order_tax_id => $tax_info) {
			...
			if ($tax_info["order_fixed_amount"] > 0) {
				$order_tax_rates[$order_tax_id]["tax_total"] += $order_fixed_amount;
			}
		}
		...
		foreach ($order_tax_rates as $order_tax_id => $tax_info) {
			...
			foreach ($tax_info as $tax_key => $tax_value) {
				if ( preg_match("/^(percent|products|shipping|tax_total|order_fixed_amount)/", $tax_key) && !preg_match("/shipping_tax_percent/", $tax_key)) {
