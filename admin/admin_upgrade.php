...
*/
	...
	$upgrade_button = str_replace("{version_number}", va_message("VA_RELEASE"), va_message("UPGRADE_BUTTON"));

	include_once("./admin_header.php");
	include_once("./admin_footer.php");

	// $last_version is version from http://www.viart.com/viart_shop.xml
	$viart_xml = @fsockopen("ssl://www.viart.com", 443, $errno, $errstr, 12);
	...
	$download_found = str_replace("{version_number}", $last_version, va_message("DOWNLOAD_FOUND_MSG"));
	$download_now = str_replace("{version_number}", $last_version, va_message("DOWNLOAD_NOW_MSG"));
	...
	if ($operation == "upgrade") 
	{
		...
		if ($ct != $session_ct) {
			...
			if (comp_vers("3.7", $current_db_version) == 1) {
				include_once("./admin_upgrade_sqls_3.7.php");
			}

			if (comp_vers("4.1", $current_db_version) == 1) {
				include_once("./admin_upgrade_sqls_4.1.php");
			}

			if (comp_vers("4.2", $current_db_version) == 1) {
				include_once("./admin_upgrade_sqls_4.2.php");
			}

			if (comp_vers("4.3", $current_db_version) == 1 || comp_vers("4.4", $current_db_version) == 1 
				|| comp_vers("4.5", $current_db_version) == 1 || comp_vers("4.6", $current_db_version) == 1
				|| comp_vers("4.7", $current_db_version) == 1 || comp_vers("4.8", $current_db_version) == 1
				|| comp_vers("4.9", $current_db_version) == 1) {
				include_once("./admin_upgrade_sqls_4.3.php");
			}

			if (comp_vers("5.1", $current_db_version) == 1) {
				include_once("./admin_upgrade_sqls_5.1.php");
			}

			if (comp_vers("5.2", $current_db_version) == 1) {
				include_once("./admin_upgrade_sqls_5.2.php");
			}

			if (comp_vers("5.3", $current_db_version) == 1) {
				include_once("./admin_upgrade_sqls_5.3.php");
			}

			if (comp_vers("5.4", $current_db_version) == 1) {
				include_once("./admin_upgrade_sqls_5.4.php");
			}

			if (comp_vers("5.5", $current_db_version) == 1) {
				include_once("./admin_upgrade_sqls_5.5.php");
			}
	...
	{
		global $db, $db_type, $table_prefix;

		if (is_array($sqls) && count($sqls)) {
			if ($db_type == "sqlsrv") {
		...
		if ($version_number) {
			...
			$db->query($sql);
			if ($db->error_desc) { $errors .= $db->error_desc . "<br>"; }
