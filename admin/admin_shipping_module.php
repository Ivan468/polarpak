...
*/
	...
	$r->add_textbox("module_order", TEXT, ADMIN_ORDER_MSG);
	...
	$r->add_textbox("user_module_name", TEXT);
	...
	elseif (strlen($shipping_module_id) && !$more_parameters)
	{
		...
		// set default values
		$sql = "SELECT MAX(module_order) FROM " . $table_prefix . "shipping_modules ";
		$module_order = get_db_value($sql) + 1;
		$r->set_value("module_order", $module_order);
