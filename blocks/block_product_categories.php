<?php
	...
	$default_title = va_message("PRODUCTS_TITLE");
	...
	$parent_id = get_setting_value($vars, "parent_id");
	...
	if ($parent_id) {
		$category_data = VA_Categories::get_category_data($parent_id);
		$category_name = get_setting_value($category_data, "category_name");
		if ($category_name) {
			$default_title = get_translation($category_name);
		}
	}
	...
	if ($categories_type == 1 || $categories_type == 2) {
	  ...
	  $t->set_file($file_block, $html_template);
		...
		$t->set_var("image_tiny",   "");
		$t->set_var("image_small",   "");
		$t->set_var("image_large",   "");
		$t->set_var("category_desc",   "");
		...
		if ($categories_type == 1) {
			...
			$sql .= " image_tiny, image_tiny_alt, image_small, image_small_alt, image_large, image_large_alt ";
			...
			$sql .= " c.image_tiny, c.image_tiny_alt, c.image_small, c.image_small_alt, c.image_large, c.image_large_alt, ";
		...
		if ($db->next_record()) {
			...
			do {
				...
				$image_tiny_alt = get_translation($db->f("image_tiny_alt"));
				...
				if (!$allowed_categories_ids || !in_array($top_category_id, $allowed_categories_ids)) {
					$top_list_class = trim($top_list_class." restricted");
				}
				$t->set_var("catalog_top_id", $top_category_id);
				$t->set_var("catalog_top_name", ($top_category_name));
				$t->set_var("category_class", htmlspecialchars($top_list_class));
				...
				if ($is_new_top) {
					...
					$images_total = 0;
					if ($categories_image == 1 && $image_tiny) {
						// show tiny image
						$images_total++;
						if (isset($restrict_categories_images) && $restrict_categories_images) { 
							$image_tiny = "image_show.php?type=tiny&category_id=".$top_category_id; 
						}
						if (!strlen($image_tiny_alt)) { $image_tiny_alt = $top_category_name; }
						$t->set_var("alt", htmlspecialchars($image_tiny_alt));
						$t->set_var("src", htmlspecialchars($image_tiny));
						$t->sparse("image_tiny", false);
					} 
					...
					if ($categories_image == 3 && $image_large) {
						...
						$t->sparse("image_large", false);
					} 	
	
					if ($images_total) {
						$t->sparse("category_image", false);
					} else {
						$t->set_var("category_image", "");
					}
		...
		if (!$pcategory) { 
			$pcategory = ($parent_id) ? $parent_id : 0; 
		}
	  ...
	  $t->set_file($file_block, $html_template);
		$t->set_var("nodes", "");
		$ajax = get_param("ajax");
		// check active categories for ajax get category_id and parent_id otherwise if it was set
		if (!$category_id && $parent_id) { $category_id = $parent_id; }

		$active_category_path = VA_Categories::get_path($category_id, 2);
		if ($parent_id && !in_array($parent_id, $active_category_path)) {
			$category_id = $parent_id;
			$active_category_path = VA_Categories::get_path($category_id, 2);
		}
		...
		if ($categories_type == 4 || $categories_type == 6) { // Tree-type structure
			$sql .= ", sc.subs_number ";
		}
		$sql .= " FROM " . $table_prefix . "categories c ";
		if ($categories_type == 4 || $categories_type == 6) { // Tree-type structure
		...
		if ($ajax) { // Ajax call for tree branch
			$top_id = $category_id;
			set_tree($categories, $category_id, $start_level, $active_category_path, $categories_image, $categories_type);
			...
			$column_width = intval(100 / $columns);
			$t->set_var("column_width", $column_width . "%");
			$top_id = ($parent_id) ? $parent_id : 0;
			//set_tree($categories, 0, 0, explode(",", $active_category_path), $categories_image, $categories_type);
			set_tree($categories, $top_id, 0, $active_category_path, $categories_image, $categories_type);
