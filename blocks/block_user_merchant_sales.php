<?php
	...
	$html_template = get_setting_value($block, "html_template", "block_user_merchant_sales.html"); 
  $t->set_file("block_body", $html_template);
	...
	$order_statuses = get_db_values("SELECT status_id, status_name FROM " . $table_prefix . "order_statuses WHERE show_for_merchant=1 AND is_active=1 ORDER BY status_order ", array(array("", "")));
	...
	$s = new VA_Sorter($settings["templates_dir"], "sorter.html", get_custom_friendly_url("user_merchant_sales.php"));
	...
	$sql .= " AND os.show_for_merchant=1 ";
	$sql .= $where;
	$sql .= " GROUP BY ";
	if ($db_type == "access" || $db_type == "sqlsrv" || $db_type == "db2") {
		if ($s_gr == 2) {
