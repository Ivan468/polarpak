<?php
	...
	$default_title = "{ADS_TITLE} {SEARCH_TITLE}";

	$html_template = get_setting_value($block, "html_template", "block_ads_search_advanced.html"); 
  $t->set_file("block_body", $html_template);
	...
	$t->set_var("s_sds", htmlspecialchars($s_sds));
	$t->set_var("s_fds", htmlspecialchars($s_fds));
	...
	if ($search_by_country) {
		...
		$countries = get_db_values("SELECT country_id,country_name FROM " . $table_prefix . "countries WHERE show_for_user=1 ORDER BY country_order, country_name ", array(array("", va_message("SELECT_COUNTRY_MSG"))));
	...
	if ($search_by_state) {
		...
		$states = get_db_values("SELECT state_id,state_name FROM " . $table_prefix . "states WHERE show_for_user=1 ORDER BY state_name ", array(array("", va_message("SELECT_STATE_MSG"))));
