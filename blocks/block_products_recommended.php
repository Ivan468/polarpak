<?php
	...
	$html_template = get_setting_value($block, "html_template", "block_products_recommended.html"); 
  $t->set_file("block_body", $html_template);
	...
	$records_per_page = get_setting_value($vars, "prod_recom_per_page", 10);
	$pages_number = 5;
	...
	if ($db->next_record())
	{
		$pr_columns = get_setting_value($vars, "prod_recom_cols", 1);
		...
		do
		{
			...
			if ($display_products != 2 || strlen($user_id)) {
				...
				if($sales_price != $price && $is_sales) {
					set_tax_price($item_id, $item_type_id, $price, 1, $sales_price, $tax_id, $tax_free, "pr_price", "pr_sales_price", "pr_tax_sales", false);

					$t->sparse("pr_price_block", false);
					$t->sparse("pr_sales", false);
				} else {
					set_tax_price($item_id, $item_type_id, $price, 1, 0, $tax_id, $tax_free, "pr_price", "", "pr_tax_price", false);
