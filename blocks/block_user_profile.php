<?php
	...
	if (!$is_ssl && $secure_user_profile && $secure_redirect && preg_match("/^https/i", $secure_url)) {
		$user_profile_url_parameters = '';
		if ($type) {
			$user_profile_url_parameters .= "?type=" . urlencode($type);
		}
		if ($return_page) {
			$user_profile_url_parameters .= (strlen($user_profile_url_parameters))? "&return_page=" . urlencode($return_page): "?return_page=" . urlencode($return_page);
		}
		header("Location: " . $user_profile_url.$user_profile_url_parameters);
		exit;
	}
	...
	if (strlen($user_id) || strlen($new_user_id)) {
		...
		$sql .= " u.login, u.name, u.first_name, u.middle_name, u.last_name, ";
	...
	$sections = array();
	$sql  = " SELECT ups.section_id, ups.step_number, ups.section_code, ups.section_name ";
	...
	$sql .= " AND (ups.user_types_all=1 OR upst.user_type_id=" . $db->tosql($type_id, INTEGER) . ") ";
	$sql .= " ORDER BY ups.section_order, ups.section_id ";
	$db->query($sql);
	while ($db->next_record()) {
		$section_id = $db->f("section_id");
		$step_number = $db->f("step_number");
		$section_code = $db->f("section_code");
		...
		if ($user_id || $step_number == $registration_last_step) {
			$sections[$section_id] = array("code" => $section_code, "name" => $section_name);
		}
	}
	...
	$sql  = " SELECT upp.* ";
	$sql .= " FROM " . $table_prefix . "user_profile_properties upp ";
	$sql .= " WHERE upp.property_id IN (" . $db->tosql($custom_ids, INTEGER_LIST).") ";
	...
	$use_random_image = get_setting_value($user_profile, "use_random_image", 1);
	...
	$global_states = get_db_values("SELECT state_id,state_name FROM " . $table_prefix . "states WHERE show_for_user=1 ", array(array("", "")));
	// get phone codes
	$phone_codes = get_phone_codes();
	$country_id = get_param("country_id");
	$delivery_country_id = get_param("delivery_country_id");
	...
	foreach ($sections as $section_id => $section_info) {
		...
		elseif ($section_code == "personal") 
		{
			...
			$r->add_textbox("middle_name", TEXT, $section_name.": ".MIDDLE_NAME_FIELD);
			...
			$r->add_textbox("address3", TEXT, $section_name.": ".STREET_THIRD_FIELD);
			$r->add_textbox("city", TEXT, $section_name.": ".CITY_FIELD);
			$r->add_textbox("province", TEXT, $section_name.": ".PROVINCE_FIELD);
			$r->add_select("state_id", INTEGER, "", $section_name.": ".STATE_FIELD);
			...
			if (isset($user_profile["show_birth_date"]) && $user_profile["show_birth_date"] == 1) {
				...
				if (isset($user_profile["birth_date_required"]) && $user_profile["birth_date_required"] == 1) {
					...
					$r->change_property("birth_date", REQUIRED, true);
				} else {
		...
		elseif ($section_code == "delivery") 
		{
			...
			$r->add_textbox("delivery_middle_name", TEXT, $section_name.": ". MIDDLE_NAME_FIELD);
			...
			$r->add_textbox("delivery_address3", TEXT, $section_name.": ". STREET_THIRD_FIELD);
			$r->add_textbox("delivery_city", TEXT, $section_name.": ". CITY_FIELD);
			$r->add_textbox("delivery_province", TEXT, $section_name.": ". PROVINCE_FIELD);
			$r->add_select("delivery_state_id", INTEGER, "", $section_name.": ". STATE_FIELD);
		...
		elseif ($section_code == "additional") 
		{
			// additional fields
	...
	// additional validation for all text fields to dissallow special symbols: < >
	foreach ($r->parameters as $param_name => $param_data) {				
		$regexp_mask = $r->get_property_value($param_name, REGEXP_MASK);
		$value_type = $r->get_property_value($param_name, VALUE_TYPE);
		if (!$regexp_mask && $value_type == TEXT && $param_name != "password" && $param_name != "confirm") {
			$r->change_property($param_name, REGEXP_MASK, "/^[^<>]+$/is");
			$r->change_property($param_name, REGEXP_ERROR, va_message("INVALID_CHARS_MESSAGE"));
		}
	}
	...
	if ($r->parameter_exists("country_code")) {
		$r->set_value("country_code", $country_code);
	}
	if ($r->parameter_exists("delivery_country_code")) {
	...
	$return_page = get_param("return_page");
	if (strlen($user_id)) {
		$edit_account_approve = get_setting_value($user_settings, "edit_account_approve", 0);
		if ($edit_account_approve) {
			$redirect_page = get_setting_value($user_profile, "update_redirect", get_custom_friendly_url("user_home.php"));
		} else {
			$redirect_page = get_setting_value($user_profile, "update_redirect", get_custom_friendly_url("user_login.php")."?operation=approve-update");
		}
	} elseif ($registration_last_step == $registration_total_steps) {
		$new_account_approve = get_setting_value($user_settings, "new_account_approve", 0);
		if (strlen($return_page)) {
			if ($ssl) {
				$redirect_page = $secure_url.$return_page;
			} else {
				$redirect_page = $site_url.$return_page;
			}
			...
			if ($new_account_approve) {
				$redirect_page = get_setting_value($user_profile, "registration_redirect", get_custom_friendly_url("user_home.php"));
			} else {
				$redirect_page = get_setting_value($user_profile, "registration_redirect", get_custom_friendly_url("user_login.php")."?operation=approve-registration");
			}
			$redirect_page = get_setting_value($user_profile, "registration_redirect", get_custom_friendly_url("user_home.php"));
		}
	} else {
		...
		if (strlen($return_page)) {
			$redirect_page .= "?return_page=".urlencode($return_page)."&ssl=".urlencode($ssl);
		}
	}
	...
	if (strlen($operation)) {
		...
		if (blacklist_check("sign_up") == "blocked") {
		...
		if (!strlen($r->errors))
		{
			...
			// check if account automatically approved
			if ($user_id) {
				$approve_account = get_setting_value($user_settings, "edit_account_approve", 0);
			} else {
				$approve_account = get_setting_value($user_settings, "new_account_approve", 0);
			}
			$r->set_value("is_approved", $approve_account);
			...
			$user_added = false; // variable to check if a new user was added
			$user_updated = false; // variable to check if existed user was updated
			if (strlen($user_id) || strlen($new_user_id))
			{
				...
				// don't update email if email used as login and email field is hidden
				if ($login_field_type == 2 && $r->parameter_exists("email") && !get_setting_value($user_profile, "show_email", 0)) {
				...
				if ($user_id && !$r->get_value("is_approved")) {
					// if account need to be re-approved then logout user automatically
					user_logout();
				} else {
					// re-login user after to update his data
					user_login("", "", $r->get_value("user_id"), 0, "", false, $errors);
				}
			} else {
			...
			$admin_update_notification = get_setting_value($user_profile, "admin_update_notification", 0);
			$user_update_notification  = get_setting_value($user_profile, "user_update_notification", 0);
			$admin_update_sms = get_setting_value($user_profile, "admin_update_sms_notification", 0);
			$user_update_sms  = get_setting_value($user_profile, "user_update_sms_notification", 0);

			if (
				($user_added && ($admin_notification || $user_notification || $admin_sms || $user_sms))
				||
				($user_updated && ($admin_update_notification || $user_update_notification || $admin_update_sms || $user_update_sms))
			) {
				$registration_date = $r->get_value("registration_date");
				$registration_date_string = va_date($datetime_show_format, $registration_date);

				// set tags for notifications
				...
				$state = $state_id ? get_array_value($state_id, $global_states) : "";
				...
				$delivery_state = $delivery_state_id ? get_array_value($delivery_state_id, $global_states) : "";
				...
				// end set of tags for notifications

				$eol = get_eol();
				if ($user_added && $admin_notification) {
					...
					$admin_subject = get_setting_value($user_profile, "admin_subject", va_constant("CUSTOMER_ACCOUNT_MSG")." ".va_constant("EVENT_ADDED_MSG"));
					$admin_message = get_setting_value($user_profile, "admin_message", va_constant("CUSTOMER_ID_MSG").": {user_id}".$eol.va_constant("CUSTOMER_EMAIL_MSG").": {email}");

					va_mail($mail_to, $admin_subject, $admin_message, $email_headers);
				}
				if ($user_added && $user_notification && $user_email) {
					$email_headers = array();
					...
					$user_subject = get_setting_value($user_profile, "user_subject", va_constant("CUSTOMER_ACCOUNT_MSG")." ".va_constant("EVENT_ADDED_MSG"));
					$user_message = get_setting_value($user_profile, "user_message", va_constant("CUSTOMER_ID_MSG").": {user_id}".$eol.va_constant("CUSTOMER_EMAIL_MSG").": {email}");

					va_mail($user_email, $user_subject, $user_message, $email_headers);
				}

				if ($user_added && $admin_sms) {
				...
				if ($user_updated && $admin_notification) {
					$mail_to = get_setting_value($user_profile, "admin_update_to", $settings["admin_email"]);
					$email_headers = array();
					$email_headers["from"] = get_setting_value($user_profile, "admin_update_from", $settings["admin_email"]);
					$email_headers["cc"] = get_setting_value($user_profile, "admin_update_cc");
					$email_headers["bcc"] = get_setting_value($user_profile, "admin_update_bcc");
					$email_headers["reply_to"] = get_setting_value($user_profile, "admin_update_reply_to");
					$email_headers["return_path"] = get_setting_value($user_profile, "admin_update_return_path");
					$email_headers["mail_type"] = get_setting_value($user_profile, "user_message_type");
  
					$user_subject = get_setting_value($user_profile, "admin_update_subject", va_constant("CUSTOMER_ACCOUNT_MSG")." - ".va_constant("EVENT_UPDATED_MSG"));
					$user_message = get_setting_value($user_profile, "admin_update_message", va_constant("CUSTOMER_ID_MSG").": {user_id}".$eol.va_constant("CUSTOMER_EMAIL_MSG").": {user_email}");
  
					va_mail($mail_to, $user_subject, $user_message, $email_headers);
				}
				if ($user_updated && $user_notification && $user_email) {
					$email_headers = array();
					$email_headers["from"] = get_setting_value($user_profile, "user_update_from", $settings["admin_email"]);
					$email_headers["cc"] = get_setting_value($user_profile, "user_update_cc");
					$email_headers["bcc"] = get_setting_value($user_profile, "user_update_bcc");
					$email_headers["reply_to"] = get_setting_value($user_profile, "user_update_reply_to");
					$email_headers["return_path"] = get_setting_value($user_profile, "user_update_return_path");
					$email_headers["mail_type"] = get_setting_value($user_profile, "user_message_type");
  
					$user_subject = get_setting_value($user_profile, "user_update_subject", va_constant("CUSTOMER_ACCOUNT_MSG")." - ".va_constant("EVENT_UPDATED_MSG"));
					$user_message = get_setting_value($user_profile, "user_update_message", va_constant("CUSTOMER_ID_MSG").": {user_id}".$eol.va_constant("CUSTOMER_EMAIL_MSG").": {user_email}");
  
					va_mail($user_email, $user_subject, $user_message, $email_headers);
				}

				if ($user_updated && $admin_update_sms) {
					$admin_update_sms_recipient  = get_setting_value($user_profile, "admin_update_sms_recipient", "");
					$admin_update_sms_originator = get_setting_value($user_profile, "admin_update_sms_originator", "");
					$admin_update_sms_message    = get_setting_value($user_profile, "admin_update_sms_message", va_constant("CUSTOMER_ACCOUNT_MSG")." - ".va_constant("EVENT_UPDATED_MSG").$eol.va_constant("CUSTOMER_ID_MSG").": {user_id}");

					parse_value($admin_update_sms_recipient); parse_value($admin_update_sms_message); parse_value($admin_update_sms_originator);

					sms_send($admin_update_sms_recipient, $admin_update_sms_message, $admin_update_sms_originator);
				}

				if ($user_updated && $user_update_sms) {
					$user_update_sms_recipient  = get_setting_value($user_profile, "user_update_sms_recipient", $r->get_value("cell_phone"));
					$user_update_sms_originator = get_setting_value($user_profile, "user_update_sms_originator", "");
					$user_update_sms_message    = get_setting_value($user_profile, "user_update_sms_message", va_constant("CUSTOMER_ACCOUNT_MSG")." - ".va_constant("EVENT_UPDATED_MSG").$eol.va_constant("CUSTOMER_ID_MSG").": {user_id}");

					parse_value($user_update_sms_recipient); parse_value($user_update_sms_message); parse_value($user_update_sms_originator);

					sms_send($user_update_sms_recipient, $user_update_sms_message, $user_update_sms_originator);
				}
			} 
		...
		// set birth_date field value to validate it correctly
		if ($r->parameter_exists("birth_date")) {
			$birth_month = $r->get_value("birth_month");
			$birth_day = $r->get_value("birth_day");
			$birth_year = $r->get_value("birth_year");
			if ($birth_year && $birth_month && $birth_day) {
				$birth_ts = mktime (0, 0, 0, $birth_month, $birth_day, $birth_year);
				$birth_date = va_time($birth_ts);
				$r->set_value("birth_date", $birth_date);
			}
		}

		// check if user need update his profile
		$r->validate();
		if ($r->errors) {
			$r->success_message = "Please update your profile:<br>".$r->errors;
			$r->errors = "";
		}
		...
		// get different states lists
		prepare_states($r);
		// set subscribe option by default
		// $r->set_value("subscribe", 1);
	}
	...
	if ($r->parameter_exists($affiliate_code_name)) {
		...
		$t->set_var("affiliate_code_help", htmlspecialchars($affiliate_code_help));
	}
	...
	foreach ($sections as $section_id => $section_info) {
		...
		if ($section_code == "login") {
			...
			for ($i = 0; $i < sizeof($parameters); $i++)
			{
				$order_param = $parameters[$i];
				$param_name = "delivery_" . $parameters[$i];
				if ($r->get_property_value($param_name, SHOW)) {
					$section_properties++;
					$order = get_setting_value($user_profile, $order_param . "_order", $section_properties);
					$displayed_profile_properties[$param_name] = $t->get_var($param_name . "_block");
					$displayed_profile_properties_orders[$param_name] = $order;
					$t->set_var($param_name . "_block", "");
				}
	...
	if ($is_subscription) {
		...
		while ($db->next_record()) {
			...
			if (strlen($user_id) || strlen($new_user_id) || strlen($operation))	{
