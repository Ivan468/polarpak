<?php 
	...
	$block_view_type = get_setting_value($vars, "block_view_type", "3");
	...
	$scale_off = get_setting_value($vars, "scale_off", 0.3);
	$slide_indent = get_setting_value($vars, "slide_indent", 1);
	$visible_slides = get_setting_value($vars, "visible_slides", 2);
	...
	$t->set_var("scale_off", htmlspecialchars($scale_off));
	$t->set_var("slide_indent", htmlspecialchars($slide_indent));
	$t->set_var("visible_slides", htmlspecialchars($visible_slides));
	...
	} else if ($slider_type == "6") { // chain
		$t->set_var("slides_class", "slides-chain");
	}
	$slides_style = "";
	...
	while ($db->next_record()) {
		...
		if ($slider_type == "6") {
			if ($row == 1) {
				...
				$t->set_var("slide_class", "slide-active");
			} else {
				$t->set_var("slide_style", "position: absolute; visibility: hidden; top: 0;");
				$t->set_var("slide_class", "slide-inactive");
			}
		} else if ($slider_type == "5") {
			if ($row == 1) {
				$t->set_var("slide_style", "");
			} else {
				$t->set_var("slide_style", "position: absolute; opacity: 0; top: 0; left: 0;");
			}
		}
