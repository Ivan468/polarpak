<?php
	...
	include_once("./includes/access_table.php");
	include_once("./includes/sites_table.php");
	...
	$html_template = get_setting_value($block, "html_template", "block_user_product.html"); 
  $t->set_file("block_body", $html_template);
	...
	$t->set_var("DISABLE_OUT_STOCK_DESC",      str_replace("{ADD_TO_CART_MSG}", va_constant("ADD_TO_CART_MSG"), va_constant("DISABLE_OUT_STOCK_DESC")));
	...
	$countries  = get_db_values("SELECT country_id,country_name FROM " . $table_prefix . "countries WHERE show_for_user=1 ORDER BY country_order ", array(array("", va_constant("SELECT_COUNTRY_MSG"))));
	...
	$categories_options[] = array("", va_constant("SELECT_CATEGORY_MSG"));
	...
	$common_fields = array("item_id", "user_id", "is_approved", "item_type_id", "item_name", "full_desc_type", "admin_id_added_by", "admin_id_modified_by", "date_added", "date_modified", "access_level", "guest_access_level", "admin_access_level", "sites_all");
		...
		if ($only_display || $save_subscriptions_by_category || $save_sites_by_category) {
			...
			if ($is_in_top) {
				if ($only_display || $save_subscriptions_by_category) {
					...
					$r->set_value("admin_access_level", VIEW_ITEMS_PERM + VIEW_CATEGORIES_ITEMS_PERM);
				}
				...
				if ($only_display || $save_subscriptions_by_category) {
					...
					$access_table->admin_access_level = $admin_access_level;
