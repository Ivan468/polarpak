<?php
	...
	$default_title = "{author_name}: {ARTICLES_TITLE}";
	...
	$html_template = get_setting_value($block, "html_template", "block_author_articles.html"); 
  $t->set_file("block_body", $html_template);
	...
	$sql .= " WHERE a.author_id=" . $db->tosql($author_id, TEXT);
	$db->query($sql);
	if ($db->next_record()) {
		$author_name = $db->f("author_name");
		$friendly_url = $db->f("friendly_url");
		if (isset($page_class) && ($page_class == "author" || $page_class == "author_details")) {
			// override canonical url if page class set as author
			if ($friendly_urls && $friendly_url) {
				$canonical_url = $friendly_url.$friendly_extension;
			} else {
	...
	$sql .= " INNER JOIN " . $table_prefix . "articles_authors aa ON a.article_id=aa.article_id) ";
	$sql .= " WHERE aa.author_id=" . $db->tosql($author_id, TEXT);
