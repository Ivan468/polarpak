<?php
	...
	if ($operation == "add") {
		...
		if ($db->next_record()) {
			do {
				...
				// add to cart
				add_to_cart($item_id, "", $price, $quantity, "wish", "ADD", $new_cart_id, $sc_errors, $sc_message, $cart_item_id, $item_name);
  ...
  $t->set_file("block_body", $html_template);
	...
	$s = new VA_Sorter($settings["templates_dir"], "sorter.html", get_custom_friendly_url("user_wishlist.php"));
	...
	$sql .= " i.tiny_image, i.tiny_image_alt, i.small_image, i.big_image, i.super_image ";
	...
	if($db->next_record())
	{
		...
		do
		{
			...
			$image_tiny = $db->f("tiny_image");
			$image_alt = $db->f("tiny_image_alt");
			if (!$image_alt) { $image_alt = $item_name; }
			$image_super = $db->f("super_image");
			if (!$image_super) { $image_super = $db->f("big_image"); }
			if (!$image_super) { $image_super = $db->f("small_image"); }
			if (!$image_super) { $image_super = $image_tiny; }


			if ($image_tiny) {
				$t->set_var("super_src", htmlspecialchars($image_super));
