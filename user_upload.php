...
*/
	...
	$upload_msg = str_replace("{button_name}", va_message("UPLOAD_BUTTON"), va_message("UPLOAD_SELECT_MSG"));
	...
	if ($filetype == "downloads" || $filetype == "previews") {
		...
		if ($downloads_dir && !preg_match("/\/|\\$/", $downloads_dir)) { $downloads_dir .= "/"; }
	...
	if ($can_select_folder) {
		...
		$subdirs    = array();
		$subdirs[0] = array("", va_message("SELECT_SUBFOLDER_MSG"));
		$i = 0;
		$selected_subdir = "";
	...
	if ($operation == "1") {
		...
		} elseif (preg_match("/\.php/i", $filename) || !(preg_match("/((\.gif)|(\.jpg)|(\.jpeg)|(\.bmp)|(\.tiff)|(\.tif)|(\.png)|(\.ico)|(\.doc)|(\.txt)|(\.flv)|(\.rtf)|(\.pdf))$/i", $filename)) ) {
			$errors = UPLOAD_FORMAT_ERROR;
		}
		...
		if (!strlen($errors))
		{
			...
			else
			{
				...
				if (strlen($errors)) {
					...
					$uploaded_msg = str_replace("{filename}", $new_filename, va_message("UPLOADED_FILE_MSG"));
