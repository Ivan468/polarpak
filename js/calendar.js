...
function buildTopCalFrame() {
    ...
    var calDoc =
        ...
        "<HEAD>" +
        "<STYLE>" +
		"select {border: 2px solid #CCCCCC; color: #666666; font: 12px Arial;}" +
        "</STYLE>" +
        "</HEAD>" +
        ...
        "<CENTER>" +
        getMonthSelect() +
        "<INPUT NAME='year' VALUE='" + calDate.getFullYear() + "'TYPE=TEXT SIZE=4 MAXLENGTH=4 onChange='parent.opener.setYear()' style='border: 2px solid #CCCCCC; color: #666666; font: 12px Arial; margin-left: 1px;'>" +
        "</CENTER>" +
        "</TD>" +
        "</TR>" +
        "<TR>" +
        "<TD COLSPAN=7>" +
        "<INPUT " +
        "TYPE=BUTTON NAME='previousYear' VALUE='<<'    onClick='parent.opener.setPreviousYear()' style='background: none; border: 2px solid #CCCCCC; color: #666666; font: 12px Arial; margin-right: 1px;'><INPUT " +
        "TYPE=BUTTON NAME='previousMonth' VALUE=' < '   onClick='parent.opener.setPreviousMonth()' style='background: none; border: 2px solid #CCCCCC; color: #666666; font: 12px Arial; margin-right: 1px;'><INPUT " +
        "TYPE=BUTTON NAME='today' VALUE='Today' onClick='parent.opener.setToday()' style='background: none; border: 2px solid #FEA500; color: #FF6600; font: 12px Arial;'><INPUT " +
        "TYPE=BUTTON NAME='nextMonth' VALUE=' > '   onClick='parent.opener.setNextMonth()' style='background: none; border: 2px solid #CCCCCC; color: #666666; font: 12px Arial; margin-left: 1px;'><INPUT " +
        "TYPE=BUTTON NAME='nextYear' VALUE='>>'    onClick='parent.opener.setNextYear()' style='background: none; border: 2px solid #CCCCCC; color: #666666; font: 12px Arial; margin-left: 1px;'>" +
        "</TD>" +
        "</TR>" +
        "</TABLE>" +
        "</CENTER>" +
...
function buildCalParts() {
    ...
    calendarBegin =
        ...
        "<STYLE type='text/css'>" +
        "<!--" +
        "TD.heading {border-bottom: 2px solid #CCCCCC; font: bold 15px Arial; padding: 0 2px 0 0;}" +
        "A.focusDay:link, A.focusDay:visited {background: white; border: 1px solid #FEA500; color: #FF6600; display: block; font: bold 15px Arial; margin: 2px 2px 0 0; text-decoration: none;}" +
        "A.focusDay:hover {text-decoration: none;}" +
        "A.weekday:link, A.weekday:visited {background: white; border: 1px solid #CCCCCC; color: #666666; display: block; font: bold 15px Arial; margin: 2px 2px 0 0; text-decoration: none;}" +
...
function returnDate(inDay)
{
    ...
    // RETURN TWO-DIGIT YEAR
		...
		// replace time to zero values
		if (calDateFormat.indexOf("HH") != -1) {
			outDate = jsReplace(outDate, "HH", "00");
		}
		if (calDateFormat.indexOf("hh") != -1) {
			outDate = jsReplace(outDate, "hh", "00");
		}
		if (calDateFormat.indexOf("mm") != -1) {
			outDate = jsReplace(outDate, "mm", "00");
		}
		if (calDateFormat.indexOf("ss") != -1) {
			outDate = jsReplace(outDate, "ss", "00");
		}
