function openUsersWindow(windowUrl, formName, fieldName, selectionType)
{
  ...
  if (selectionType != "") {
	...
	var usersWin = window.open (windowUrl + queryString, 'usersWin', 'toolbar=no,location=no,directories=no,status=yes,menubar=no,scrollbars=yes,resizable=yes,width=800,height=600');
	usersWin.focus();
}
...
function selectUser(userId, userName, userEmail)
{
	if (window.opener) {
		...
		window.opener.setUser(userId, userName, userEmail, formName, fieldName, selectionType);
		window.opener.focus();
	}
	window.close();
}
...
function setUser(userId, userName, userEmail, formName, fieldName, selectionType)
{
	if (selectionType == "single") {
		var idControl = document.forms[formName].elements[fieldName];
		var userNameObj = document.getElementById("userName");
		idControl.value = userId;
		var userTemplate = ""; var userHTML = "";
		var userTemplateObj = document.getElementById("userTemplate");
		if (userTemplateObj) {
			userTemplate = userTemplateObj.innerHTML;
			userHTML = userTemplate.replace(/\[user_id\]/gi, userId); 
			userHTML = userHTML.replace(/\[user_name\]/gi, userName); 
			userHTML = userHTML.replace(/\[user_email\]/gi, userEmail); 
		} else {
			userHTML  = "<a href=\""+userViewLink+"?user_id="+userId+"\" class=\"title\" target=\"_blank\">"+userName+"</a>";
			userHTML += " (#"+userId+") - <a href=\"#\" onClick=\"removeSingleUser('" + id + "', '" + formName + "', '" + fieldName + "'); return false;\">" + removeButton + "</a> | ";
		}
		userNameObj.innerHTML = userHTML;
	} else {
		var userAdded = false;
		var usersArray = "";
