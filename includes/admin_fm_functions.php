...
function read_current_dir($root_dir, $conf_array_dirs)  {
	global $conf_root_dir;
	$content_cur_dir = array();
	if(is_dir($root_dir)) {
...
function check_dir_path($path_dir) {
	...
	$top_dir = $dirs[0];
	if(!in_array($top_dir, $conf_array_dirs) || !is_dir($top_dir)) { $path_dir = ""; }
	return $path_dir;
}
...
function fm_errors($code, $parameter = "", $parameter2 = ""){
	switch ($code) {
		...
		case 113 :
			...
			break;
		case 114 :
			$error = va_constant("NO_FILE_UPLOADED_MSG")." ".$parameter;
			break;
		case 115 :
			$error = va_constant("INVALID_FILE_EXTENSION_MSG")." ".$parameter;
			break;
		case 116 :
