...
class VA_Tree
{
	...
	//-- constructor
	public function __construct($db_category_id, $db_category_name, $db_parent_id, $db_table, $tree_name, $top_name = "Top", $erase_tags = true)
	{
	...
	function get_path($category_id)
	{
		global $db;
		$path = ""; 
		$path_ids = array(); // save used ids to prevent unlimited recursion 
		if($category_id) {
		...
		else 
		{
			...
			while ($category_id)
			{
				...
				if($db->next_record())
				{
					...
					if ($this->erase_tags) { $category_name = strip_tags($category_name); }
					if ($current_id != $category_id)
					{
						$t->set_var($tree_name . "_cat_id", htmlspecialchars($category_id));
					...
					else
					{
						...
						$t->set_var($tree_name . "_current_id", htmlspecialchars($category_id));
					}
					$category_id = $db->f($this->db_parent_id);
				}
				else
