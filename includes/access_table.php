...
*/
	...
	class VA_Access_Table {
		...
		public function __construct($template_path, $filename, $field_id = ""){
		...
		function set_tables($table_name, $user_types_table_name, $subscriptions_table_name, $field_name, $recursion_field, $field_value, $sql = false) {
			...
			$this->admin_selected_access_level = 0;
			...
			if (!count($this->user_types) && !count($this->subscriptions)) {
				...
				$this->admin_selected_access_level = $max_access_level;
				return false;
			}
			...
			if ($operation) {
				foreach ($this->access_levels_keys AS $access_level_key) {
					...
					$access_level = get_param($this->field_id . "_admin_" . $access_level_key);
					if ($access_level) {
						$this->admin_selected_access_level += $access_level_key;
					}
				}
		...
		function parse($var, $all_access_level = 255, $guest_access_level = 255, $admin_access_level = 255){
			global $t, $db, $table_prefix, $eol;
			$eol = get_eol();

			$all_access_level = intval($all_access_level);
			$guest_access_level = intval($guest_access_level);
			$admin_access_level = intval($admin_access_level);
			$this->all_selected_access_level   = $all_access_level;
			$this->guest_selected_access_level = $guest_access_level;
			$this->admin_selected_access_level = $admin_access_level;
			if (!count($this->user_types) && !count($this->subscriptions)) {
				$max_access_level = 0;
				foreach ($this->access_levels_keys AS $access_level_key) {
					$max_access_level += $access_level_key;
			...
			foreach ($this->access_levels AS $level_value => $level) {
				...
				$checked = ($level_value&$admin_access_level) ? "checked" : "";
				$this->t->set_var("checked", $checked);
				$this->t->parse("admin_level_block");			
			}			
			$this->t->parse("header_block");
			$this->t->parse("all_block");
			$this->t->parse("guest_block");
			$this->t->parse("admin_block");
			...
			foreach ($this->user_types AS $type_id => $type_name) {
				...
				$ti++;
				if (isset($this->selected_user_access_levels[$type_id])) {
					$acl = intval($this->selected_user_access_levels[$type_id]);
				} else {
					$acl = 0;
				}
				...
				if ($type_id) {
					...
					if (isset($this->subscriptions[$type_id])) {
						foreach ($this->subscriptions[$type_id] AS $subscription_id => $subscription_name) {
							...
							$si++; $tsi++;
							if (isset($this->selected_access_levels[$subscription_id])) {
								$acl = intval($this->selected_access_levels[$subscription_id]);
							} else {
								$acl = 0;
							}
							$row_class = "row" . ($row_index % 2 ? "1" : "2");
		...
		function save_values_recursive($field_value) {
			...
			if ($nested) {
				...
				$sql .= " , admin_access_level= " . $db->tosql($this->admin_selected_access_level, INTEGER, true, false);
		...
		function save_array_values($array, $access_level = 1, $guest_access_level = 1, $admin_access_level = 1) {
			global $db, $table_prefix;
			...
			$sql .= ", admin_access_level = " . $db->tosql($admin_access_level, INTEGER);
