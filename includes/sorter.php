...
class VA_Sorter
{
	...
	var $t;                         // template with sorter structure
	var $order_by;                  
	var $order_columns;                  
	var $is_column;
	var $is_column_link;
  ...
  public function __construct($template_path, $filename, $sorter_page, $sorter_parameter = "sort", $remove_parameters = "", $pass_parameters = "")
  {
	...
	function set_sorter($column_name, $sorter_name, $sorter_value, $table_column, $column_asc = "", $column_desc = "", $is_default = false)
	{
		global $t;
		...
		//$page .= $sorter_order;
		$page_asc  = htmlspecialchars($page.$query_string."&".$this->sorter_parameter."_dir=asc");
		$page_desc = htmlspecialchars($page.$query_string."&".$this->sorter_parameter."_dir=desc");

		$this->t->set_var("column_name", $column_name);

		$this->t->set_var("column_on", ""); $this->t->set_var("column_off", "");
		$this->t->set_var("image_on_asc", ""); $this->t->set_var("image_off_asc", ""); 
		$this->t->set_var("image_on_desc", ""); $this->t->set_var("image_off_desc", ""); 
		$sorted_value = get_param($this->sorter_parameter . "_ord");
		if($sorted_value == $sorter_value 
			|| (($this->default_sorter_value == $sorter_value || $is_default) && !strlen($sorted_value)))
		{
			$direction = strtolower(get_param($this->sorter_parameter . "_dir"));
			if(!$direction) { $direction = $this->default_sorter_order; }
			if($this->is_column)
